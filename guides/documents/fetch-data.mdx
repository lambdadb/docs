---
title: Fetch data
description: Retrieve documents by ID from a LambdaDB collection. Supports consistent reads, vector inclusion, field filtering, and partition filters.
---
...
<PythonClientLifecycleNote />
  ...
  ```bash cURL
  curl -X POST "$BASE_URL/projects/$PROJECT_NAME/collections/${collection_name}/docs/fetch" \
    -H "Content-Type: application/json" \
    -H "x-api-key: ${YOUR_API_KEY}" \
    -d '{
      "ids": ["33201222"],
      "ref": { "kind": "tag", "name": "validated-2026-09" },
      "includeVectors": true,
      "fields": {
...
`consistentRead: true` is supported only when `ref` directly selects a branch. Tag and alias reads reject consistent reads. See [Branches, tags, and aliases](/guides/data-versioning/branches-tags-aliases#read-from-a-ref).

Consistent reads overlay eligible pending writes. Pending bulk imports remain invisible until committed, and a pending payload above the consistent-read limit returns `429`.
