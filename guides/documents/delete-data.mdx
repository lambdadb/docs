---
title: Delete data
description: Remove documents from a LambdaDB collection by specifying document IDs. Includes Python, TypeScript, Go, and cURL deletion code examples.
---
...
The REST request accepts an optional `branch` field. Omitting it deletes from the default branch, `main`; a tag or alias is not a valid write target.

Specify exactly one of `ids` or `filter`. An empty request, both fields together, or `partitionFilter` alone returns `400 BadRequest`. For partitioned collections, `partitionFilter` narrows the IDs or filter selection; it is not an independent deletion selector.
...
These snippets reuse the imports and client setup above. For Go, replace the ID-deletion call inside `main` with the filter example below.
...
<CodeGroup>
  ...
  filter := map[string]any{
    "queryString": map[string]any{"query": "genre:documentary AND year:2019"},
  }
...
The delete operation makes the logical collection unavailable before returning `200 OK`, so the same collection name can be reused immediately.
