...
Create a **project API key** in [LambdaDB Cloud](https://app.lambdadb.ai): open your project's **API Keys** tab and select **Create key**. Save the full key value when it is shown, and copy the project's **base URL** and **project name**. Project creation does not automatically issue a key. This guide assumes the `cohere-wikipedia-en-100k` collection already exists in that project with `title`, `url`, `text`, and `vector` fields. For signup, permissions, and key setup, see the [Quickstart](/guides/get-started/quickstart).
...
class WikipediaSearchExamples:
    ...
    def run_all_examples(self):
        ...
        print("🚀 Running LambdaDB Search Examples")
        print("=" * 60)
