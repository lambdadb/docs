...
Shards are an internal execution detail; LambdaDB does not expose shard-count or placement settings to users. Separately, applications can [configure collection partitions](/guides/collections/create-a-collection#create-a-partitioned-collection) and use `partitionFilter` to narrow a request's physical search scope.
...
On a partitioned collection, `partitionFilter` restricts the search to selected physical partitions; unselected partitions are not searched. Without it, all partitions remain in the search scope. Internal sharding distributes the work within that scope rather than selecting the application's tenant, session, or repository data.
