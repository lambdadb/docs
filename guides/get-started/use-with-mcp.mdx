...
- Your **project name**
- Node.js **20.7.0 or later** and npm
...
Clone the LambdaDB MCP server's `main` branch, then build it. The current server uses LambdaDB TypeScript SDK 0.5.1, including automatic large-result downloads.

```bash
git clone --branch main https://github.com/lambdadb/lambdadb-mcp
cd lambdadb-mcp
npm ci
npm run build
```

If you are using the repository's local environment workflow, you can also create a local `.env.local` file:
...
For local execution with this file, run `npm run start:env`. The launcher loads `.env.local` when present, otherwise `.env`; exported environment variables take precedence. Direct `node dist/index.js` execution does not load these files automatically. The MCP client configuration below supplies its own environment variables.
