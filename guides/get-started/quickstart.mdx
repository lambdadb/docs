...
// Create collection with text and vector indexes
const res = await lambdaDB.collections.create({
...
// Upload documents to the collection
const res = await lambdaDB.collections.docs.upsert({
    collectionName: collectionName,
    requestBody: {
        docs: docs
    }
...
```typescript TypeScript
// Check collection status and document count
const res = await lambdaDB.collections.get({collectionName: collectionName})
console.log(res)
```
</CodeGroup>
...
```
```typescript TypeScript
const res = await lambdaDB.collections.query({
    collectionName: collectionName,
    requestBody: {
        size: 10,
        query: {
...
```
```typescript TypeScript
await lambdaDB.collections.delete({collectionName: collectionName})
```
</CodeGroup>
