...
A query that combines vector search with traditional text-based queries to achieve better relevance by leveraging both semantic similarity and keyword matching.
...
| l2     | L2 norm Euclidean distance-based normalization       | Yes           |
...
|-----------|--------------------------------|--------|----------|---------|
| query     | Query object (queryString/knn) | object | ✓        |         |
...
This example uses Reciprocal Rank Fusion without boost parameters to balance text and vector search results.
