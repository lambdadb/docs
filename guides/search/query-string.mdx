...
On `text` fields, ordinary search terms are processed using the field's configured [text analyzers](/guides/collections/choose-text-analyzers). Setting `skipSyntax: true` disables query syntax parsing but still applies text analysis.
