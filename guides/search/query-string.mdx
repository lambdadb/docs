LambdaDB supports a simple query string query that uses the [Lucene Query Syntax](https://lucene.apache.org/core/10_2_2/queryparser/org/apache/lucene/queryparser/flexible/standard/StandardQueryParser.html) to search documents.
...
> **Note:** A query to a collection with `object` field type requires special handling.
...
Note: While keyword fields are technically supported, range queries on text or keyword fields may not execute if search.allow_expensive_queries is disabled for performance reasons.
