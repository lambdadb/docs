...
Get detailed information about a specific collection, including its configuration, document count, default branch, and retention period.
...
Retention must be an integer from `1` through `31`. Set `tags` to `{}` to clear
all collection metadata tags. An update request must include at least one
supported field.
