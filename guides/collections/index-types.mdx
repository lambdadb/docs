...
<Warning>
The dot (.) character cannot be used as an index name and you can only add indexes to existing collections. Modifying or deleting existing indexes is not supported.
</Warning>
...
`keyword` type is used for structured content such as IDs, email addresses, hostnames, status codes, zip codes, or tags. Keyword indexes are often used in sorting, aggregations, and term-level queries. Note that LambdaDB does not index any string longer than 4096 characters.
...
This type is for date and time in [RFC 3339 format](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6). `datetime` indexes are optimized for sorting and range queries, or to search for a specific time.
...
A k-nearest neighbor (kNN) search finds the k nearest vectors to a query vector, as measured by a similarity metric. LambdaDB supports four similarity metrics: `euclidean`, `dot_product`, `cosine`, `max_inner_product`. You can define the vector similarity to use in kNN search.

LambdaDB supports `multi-vector field search`, allowing you to perform kNN searches across multiple vector fields simultaneously within a single query. This enables complex semantic search scenarios where you can combine different types of embeddings (e.g., text embeddings, image embeddings) in one search operation.
