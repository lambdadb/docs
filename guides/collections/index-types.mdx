---
title: Index types
description: Reference for LambdaDB's nine index types — text, keyword, vector, sparse vector, boolean, datetime, long, double, and object — with analyzer options.
---
...
Choose one or more [supported analyzers](#supported-analyzers) for each text field. Omitting `analyzers` uses `["standard"]`. LambdaDB applies every analyzer you select; it does not automatically detect the language. See [Choose text analyzers](/guides/collections/choose-text-analyzers) for language recommendations, Chinese examples, and the tradeoffs of using multiple analyzers.
...
`keyword` type is used for structured content such as IDs, email addresses, hostnames, status codes, zip codes, or tags. Use keyword indexes for sorting and exact-value filters. The documented query API does not provide aggregation operations.
...
|:---------|:------------|
| `standard` | Default general-purpose Unicode text analysis |
| `english` | English |
| `korean` | Korean (Nori) |
| `japanese` | Japanese (Kuromoji) |
| `chinese` | Simplified Chinese word segmentation (SmartChinese); also processes mixed Chinese-English text |
| `cjk` | Overlapping two-character tokens for Chinese, Japanese, and Korean text |
| `arabic` | Arabic |
| `french` | French |
| `german` | German |
| `hindi` | Hindi |
...
Use the lowercase names above and avoid duplicates. Omit `analyzers` for the default, or select at least one analyzer; an empty list does not use the default. See [Choose text analyzers](/guides/collections/choose-text-analyzers) before combining analyzers or migrating an existing field.

<Note>

Ask in the [LambdaDB community Slack](https://join.slack.com/t/lambdadbcommunity/shared_invite/zt-3sg7565zm-sCTW3odRkEQt~auWUVVsTw) or email [support@lambdadb.ai](mailto:support@lambdadb.ai) if you need an analyzer not listed above.
...
| `dot_product` | Dot product for unit-length vectors | Document and query vectors are already normalized to unit length |
| `max_inner_product` | Maximum inner product without a normalization requirement | Preserve vector magnitudes when they contribute to relevance |

For dense vectors, use `dot_product` only when both document and query vectors have unit length. Use `max_inner_product` to preserve an unnormalized inner-product objective; normalizing vectors can change that ranking. These semantics follow [Lucene's vector similarity definitions](https://lucene.apache.org/core/10_4_0/core/org/apache/lucene/index/VectorSimilarityFunction.html). This normalization requirement does not apply to `sparseVector` fields.

Dense-vector query results contain relevance scores, not raw distances or inner products. Validate score thresholds separately when migrating between systems.
