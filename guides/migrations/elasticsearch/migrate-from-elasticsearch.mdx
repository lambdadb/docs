...
Follow [Install the Migration CLI](/guides/migrations/overview#install-the-cli) to install stable `v0.1.6` using Homebrew or the standalone installer. Check `lambdadb-migration --version` and `lambdadb-migration elasticsearch --help` before continuing.
...
- [choose LambdaDB text analyzers](/guides/collections/choose-text-analyzers) when your Elasticsearch workload depends on language-specific analysis; review their matching behavior rather than assuming equivalent Elasticsearch configuration
...
Not sure how to map your Elasticsearch workload to LambdaDB? Email [support@lambdadb.ai](mailto:support@lambdadb.ai) or ask in the [LambdaDB community Slack](https://join.slack.com/t/lambdadbcommunity/shared_invite/zt-3sg7565zm-sCTW3odRkEQt~auWUVVsTw) for help with field mappings, query compatibility, and migration next steps.

A brief description of your Elasticsearch version, index and document counts, search features, and the problem you want to solve is enough to start. If useful, include a sanitized mapping, Query DSL example, or CLI error so we can discuss the mapping in concrete terms.
