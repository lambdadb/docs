...
| Source | Status | Notes |
|:-------|:-------|:------|
| Qdrant | Supported in CLI `v0.1.4` | Dense vectors, named dense vectors, sparse vectors, payload indexes, local checkpoints, validation reports, and dense/sparse-vector query overlap checks. |
| Pinecone Serverless | Supported in CLI `v0.1.4` | Dense indexes, sparse indexes, namespaces, ID-prefix scoped migrations, metadata payloads, local checkpoints, validation reports, and dense/sparse-vector query overlap checks. |
...
For vector migrations, `--migration.query-overlap` compares source and LambdaDB nearest-neighbor results for validation samples. By default, it reports overlap without failing the migration. Set `--migration.query-overlap-min-ratio` above `0` to require a minimum average overlap.
...
<CardGroup cols={2}>
  ...
  <Card title="Migrate from Pinecone" href="/guides/migrations/migrate-from-pinecone" icon="arrows-rotate">
