...
Internal query sharding is managed by LambdaDB regardless of this logical layout. The optional [field-based partition configuration](/guides/collections/create-a-collection#create-a-partitioned-collection) is a separate application choice; it does not require you to operate the underlying query shards.
...
| `WHERE tenant_id = $1 AND status = $2` combined with vector ordering | Rewrite required | Index the scope fields as appropriate scalar types and express the predicates inside `knn.filter`, using `bool` for combinations. Resolve the allowed tenant in the backend. |
