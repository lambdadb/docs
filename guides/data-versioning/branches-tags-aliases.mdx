...
Deleting an alias target does not delete the alias. A missing target sets `dangling` to `true`; reads through that alias return `400 BadRequest` until it is retargeted to an existing branch or tag. Selecting a ref that does not exist returns `404 ResourceNotFound`.

Aliases bind to the target's identity. Recreating a branch or tag under the same name does not repair a dangling alias; explicitly retarget it to the new target.
