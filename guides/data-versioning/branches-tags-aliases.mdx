...
LambdaDB data versioning provides named document histories inside a collection. Every collection starts with a default branch named `main`.
...
Send every header returned by the upload-URL response unchanged with the presigned `PUT`.
