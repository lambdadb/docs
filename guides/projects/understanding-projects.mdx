...
A LambdaDB project belongs to an organization and contains a number of collections.
...
<Note>
  ...
  Making calls to `projects` APIs requires the organization admin API key.
...
Read, write, and inference quotas are measured over the configured `period`.
Storage quota is measured against current physical storage.
Inference quota is measured in LIU (LambdaDB Inference Unit), which is used for managed model usage such as managed embeddings.
